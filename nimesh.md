...
What to build
An HTTP API that accepts a job (command, env, timeout)
A background runner that executes it as a subprocess
Storage so you can query the job's status, exit code, and logs later
